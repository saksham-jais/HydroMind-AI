...
function useContacts(region?: string) {
  return useQuery<Contact[]>({
    ...
    queryFn: async () => {
      const url = region ? `${API}/contacts?region=${encodeURIComponent(region)}` : `${API}/contacts`;
...
function ContactModal({
...
}: { open: boolean; onClose: () => void; initial?: Contact | null; onSave: (data: Omit<Contact, "id">) => Promise<void> }) {
  ...
  useEffect(() => {
    setForm(initial ? { name: initial.name, phone: initial.phone, region: initial.region, note: initial.note } : EMPTY);
  }, [initial, open]);
                ...
                {regionContacts.map((c) => (
                  <div key={c.id} className="flex items-start gap-3 rounded-lg border border-border bg-card p-4 transition hover:border-primary/40">
                    ...
                    <div className="flex flex-col gap-1.5 shrink-0">
                      <Button size="sm" variant="outline" className="h-7 w-7 p-0" onClick={() => { setEditing(c); setModalOpen(true); }}>
                        <Pencil className="h-3 w-3" />
                      </Button>
                      <Button size="sm" variant="outline" className="h-7 w-7 p-0 text-red-400 hover:bg-red-500/10 hover:text-red-400" onClick={() => setDeleteId(c.id)}>
