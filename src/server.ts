// Inlined error capture — avoids a separate Vite module fetch that can timeout on Windows dev.

let lastCapturedError: { error: unknown; at: number } | undefined;
const ERROR_TTL_MS = 5_000;

function recordCapturedError(error: unknown) {
  lastCapturedError = { error, at: Date.now() };
}

function consumeLastCapturedError(): unknown {
  if (!lastCapturedError) return undefined;
  if (Date.now() - lastCapturedError.at > ERROR_TTL_MS) {
